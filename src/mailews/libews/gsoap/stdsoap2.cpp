...
{ if (soap->bufidx < soap->buflen)
  ...
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "1.Read %u bytes from socket=%d/fd=%d\n", (unsigned int)soap->buflen, soap->socket, soap->recvfd));
...
zlib_again:
  ...
  { soap->bufidx = 0;
    ...
    DBGLOG(TEST, SOAP_MESSAGE(fdebug, "2.Read %u bytes from socket=%d/fd=%d\n", (unsigned int)ret, soap->socket, soap->recvfd));
    DBGMSG(RECV, soap->buf, ret);
  }
#ifdef WITH_ZLIB
  if (soap->mode & SOAP_ENC_ZLIB)
...
extern "C"
bool soap_create_local_time;

bool soap_create_local_time = false;
...
soap_dateTime2s(struct soap *soap, time_t n)
{
    ...
    //STONE: task item needs to use local time when do create/update
    if (soap_create_local_time) {
        if ((pT = localtime(&n)))
