...
#else
    mailews::MsgI18NConvertFromUnicode(mailews::MsgI18NFileSystemCharset(), str, native);
    mailews::MsgI18NConvertToUnicode(mailews::MsgI18NFileSystemCharset(), native, roundTripped);
#endif
...
#else
  return mailews::MsgI18NConvertFromUnicode(mailews::MsgI18NFileSystemCharset(), path, aPathCString);
#endif
}

bool NS_MsgStripRE(const char **stringP, uint32_t *lengthP, nsCString & modifiedSubject)
{
  const char *s, *s_end;
  uint32_t L;
  bool result = false;
...
{
  MsgFileStream *newFileStream = new MsgFileStream;
...
{
  MsgFileStream *msgFileStream = static_cast<MsgFileStream *>(fileStream);
  if (msgFileStream)
...
protected:
  virtual ~MsgInterfaceRequestorAgg() {
  }
