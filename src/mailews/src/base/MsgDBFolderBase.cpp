...
nsICollation * MsgDBFolderBase::gCollationKeyGenerator = nullptr;

char16_t *MsgDBFolderBase::kLocalizedInboxName;
char16_t *MsgDBFolderBase::kLocalizedTrashName;
char16_t *MsgDBFolderBase::kLocalizedSentName;
char16_t *MsgDBFolderBase::kLocalizedDraftsName;
char16_t *MsgDBFolderBase::kLocalizedTemplatesName;
char16_t *MsgDBFolderBase::kLocalizedUnsentName;
char16_t *MsgDBFolderBase::kLocalizedJunkName;
char16_t *MsgDBFolderBase::kLocalizedArchivesName;

char16_t *MsgDBFolderBase::kLocalizedBrandShortName;

nsrefcnt MsgDBFolderBase::mInstanceCount=0;

NS_IMPL_ISUPPORTS_INHERITED(MsgDBFolderBase, nsRDFResource, 
                             nsISupportsWeakReference, nsIMsgFolder,
                             nsIDBChangeListener, nsIUrlListener,
                             nsIJunkMailClassificationListener,
                             nsIMsgTraitClassificationListener)

#define MSGDBFOLDER_ATOM(name_, value_) nsIAtom* MsgDBFolderBase::name_ = nullptr;
...
const nsStaticAtom MsgDBFolderBase::folder_atoms[] = {
#define MSGDBFOLDER_ATOM(name_, value_) NS_STATIC_ATOM(name_##_buffer, &MsgDBFolderBase::name_),
#include "nsMsgDBFolderAtomList.h"
#undef MSGDBFOLDER_ATOM
};
#endif

MsgDBFolderBase::MsgDBFolderBase(void)
: mAddListener(true),
  mNewMessages(false),
...
#else
#define MSGDBFOLDER_ATOM(name_, value_) name_ = mailews::MsgNewPermanentAtom(value_);
#include "nsMsgDBFolderAtomList.h"
#undef MSGDBFOLDER_ATOM
#endif
    initializeStrings();
  ...
  for (uint32_t i = 0; i < nsMsgProcessingFlags::NumberOfFlags; i++)
    mProcessingFlag[i].keys = MsgKeySetU::Create();
}
...
NS_IMETHODIMP MsgDBFolderBase::Shutdown(bool shutdownChildren)
{
  if(mDatabase)
  {
...
NS_IMETHODIMP MsgDBFolderBase::ForceDBClosed()
{
  int32_t count = mSubFolders.Count();
  for (int32_t i = 0; i < count; i++)
...
NS_IMETHODIMP MsgDBFolderBase::CloseAndBackupFolderDB(const nsACString& newName)
{
  ForceDBClosed();
  ...
  rv = mailews::GetSummaryFileLocation(folderPath, getter_AddRefs(dbFile));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP MsgDBFolderBase::OpenBackupMsgDatabase()
{
  if (mBackupDatabase)
    return NS_OK;
  nsCOMPtr<nsIFile> folderPath;
...
NS_IMETHODIMP MsgDBFolderBase::RemoveBackupMsgDatabase()
{
  nsCOMPtr<nsIFile> folderPath;
  nsresult rv = GetFilePath(getter_AddRefs(folderPath));
  NS_ENSURE_SUCCESS(rv, rv);
  ...
  nsCOMPtr<nsIFile> backupDBFile;
  rv = mailews::GetSummaryFileLocation(backupDBDummyFolder, getter_AddRefs(backupDBFile));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP MsgDBFolderBase::StartFolderLoading(void)
{
  if(mDatabase)
    mDatabase->RemoveListener(this);
  mAddListener = false;
...
NS_IMETHODIMP MsgDBFolderBase::EndFolderLoading(void)
{
  if(mDatabase)
...
NS_IMETHODIMP
MsgDBFolderBase::GetExpungedBytes(int64_t *count)
{
  ...
  if (mDatabase)
  {
    ...
    rv = folderInfo->GetExpungedBytes((int64_t *) count);
    if (NS_SUCCEEDED(rv))
...
NS_IMETHODIMP MsgDBFolderBase::GetCharset(nsACString& aCharset)
{
  nsCOMPtr<nsIDBFolderInfo> folderInfo;
...
NS_IMETHODIMP MsgDBFolderBase::SetCharset(const nsACString& aCharset)
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::GetCharsetOverride(bool *aCharsetOverride)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetCharsetOverride(bool aCharsetOverride)
{
  nsresult rv;
  nsCOMPtr<nsIDBFolderInfo> folderInfo;
  nsCOMPtr<nsIMsgDatabase> db;
...
NS_IMETHODIMP MsgDBFolderBase::GetHasNewMessages(bool *hasNewMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetHasNewMessages(bool curNewMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetHasFolderOrSubfolderNewMessages(bool *aResult)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetGettingNewMessages(bool *gettingNewMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetGettingNewMessages(bool gettingNewMessages)
{
  mGettingNewMessages = gettingNewMessages;
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetFirstNewMessage(nsIMsgDBHdr **firstNewMessage)
{
...
NS_IMETHODIMP MsgDBFolderBase::ClearNewMessages()
{
  nsresult rv = NS_OK;
...
void MsgDBFolderBase::UpdateNewMessages()
{
...
// MsgDBFolderBase. If it lived at a higher level, we could cache the account manager and folder cache.
nsresult MsgDBFolderBase::GetFolderCacheElemFromFile(nsIFile *file, nsIMsgFolderCacheElement **cacheElement)
{
  nsresult result;
...
nsresult MsgDBFolderBase::ReadDBFolderInfo(bool force)
{
  ...
  if (force || !mInitializedFromCache)
  {
    ...
    if(NS_SUCCEEDED(result))
    {
      ...
      if (folderInfo)
      {
        ...
        folderInfo->GetExpungedBytes((int64_t *)&mExpungedBytes);
...
nsresult MsgDBFolderBase::SendFlagNotifications(nsIMsgDBHdr *item, uint32_t oldFlags, uint32_t newFlags)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP MsgDBFolderBase::DownloadMessagesForOffline(nsIArray *messages, nsIMsgWindow *)
{
  NS_ASSERTION(false, "imap and news need to override this");
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::DownloadAllForOffline(nsIUrlListener *listener, nsIMsgWindow *msgWindow)
{
  NS_ASSERTION(false, "imap and news need to override this");
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetMsgStore(nsIMsgPluggableStore **aStore)
{
...
/*
nsresult MsgDBFolderBase::GetSummaryFile(nsIFile** aSummaryFile)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetOfflineStoreInputStream(nsIInputStream **stream)
{
...
bool MsgDBFolderBase::VerifyOfflineMessage(nsIMsgDBHdr *msgHdr, nsIInputStream *fileStream)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetOfflineFileStream(nsMsgKey msgKey, int64_t *offset, uint32_t *size, nsIInputStream **aFileStream)
{
  ...
  if (seekableStream)
  {
    seekableStream->Tell(offset);
        ...
        else
        {
          ...
          // skip "From "/FCC line
          bool foundNextLine = mailews::MsgAdvanceToNextLine(startOfMsg, msgOffset, bytesRead - 1);
          if (foundNextLine && !strncmp(startOfMsg + msgOffset,
                                        X_MOZILLA_STATUS, X_MOZILLA_STATUS_LEN))
          {
            // skip X-Mozilla-Status line
            if (mailews::MsgAdvanceToNextLine(startOfMsg, msgOffset, bytesRead - 1))
            {
              if (!strncmp(startOfMsg + msgOffset,
                           X_MOZILLA_STATUS2, X_MOZILLA_STATUS2_LEN))
                mailews::MsgAdvanceToNextLine(startOfMsg, msgOffset, bytesRead - 1);
            }
          }
          int32_t findPos = mailews::MsgFindCharInSet(nsDependentCString(startOfMsg),
                                             ":\n\r", msgOffset);
...
NS_IMETHODIMP
MsgDBFolderBase::GetOfflineStoreOutputStream(nsIMsgDBHdr *aHdr,
...
NS_IMETHODIMP
MsgDBFolderBase::GetMsgInputStream(nsIMsgDBHdr *aMsgHdr, bool *aReusable,
...
nsresult MsgDBFolderBase::CreateFileForDB(const nsAString& userLeafName, nsIFile *path, nsIFile **dbFile)
{
  NS_ENSURE_ARG_POINTER(dbFile);

  nsAutoString proposedDBName(userLeafName);
  mailews::NS_MsgHashIfNecessary(proposedDBName);
...
NS_IMETHODIMP
MsgDBFolderBase::GetMsgDatabase(nsIMsgDatabase** aMsgDatabase)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetMsgDatabase(nsIMsgDatabase *aMsgDatabase)
{
  if (mDatabase)
  {
...
NS_IMETHODIMP
MsgDBFolderBase::GetBackupMsgDatabase(nsIMsgDatabase** aMsgDatabase)
{
  NS_ENSURE_ARG_POINTER(aMsgDatabase);
...
NS_IMETHODIMP
MsgDBFolderBase::GetDBFolderInfoAndDB(nsIDBFolderInfo **folderInfo, nsIMsgDatabase **database)
{
...
NS_IMETHODIMP
MsgDBFolderBase::OnReadChanged(nsIDBChangeListener * aInstigator)
{
...
NS_IMETHODIMP
MsgDBFolderBase::OnJunkScoreChanged(nsIDBChangeListener * aInstigator)
{
...
NS_IMETHODIMP
MsgDBFolderBase::OnHdrPropertyChanged(nsIMsgDBHdr *aHdrToChange, bool aPreChange, uint32_t *aStatus, 
...
NS_IMETHODIMP MsgDBFolderBase::OnHdrFlagsChanged(nsIMsgDBHdr *aHdrChanged, uint32_t aOldFlags, uint32_t aNewFlags,
                                         nsIDBChangeListener * aInstigator)
{
  if(aHdrChanged)
  {
...
nsresult MsgDBFolderBase::CheckWithNewMessagesStatus(bool messageAdded)
{
  bool hasNewMessages;
  if (messageAdded)
...
NS_IMETHODIMP MsgDBFolderBase::OnHdrDeleted(nsIMsgDBHdr *aHdrChanged, nsMsgKey  aParentKey, int32_t aFlags,
                          nsIDBChangeListener * aInstigator)
{
...
NS_IMETHODIMP MsgDBFolderBase::OnHdrAdded(nsIMsgDBHdr *aHdrChanged, nsMsgKey  aParentKey , int32_t aFlags,
                        nsIDBChangeListener * aInstigator)
{
...
nsresult MsgDBFolderBase::OnHdrAddedOrDeleted(nsIMsgDBHdr *aHdrChanged, bool added)
{
  if(added)
...
NS_IMETHODIMP MsgDBFolderBase::OnParentChanged(nsMsgKey aKeyChanged, nsMsgKey oldParent, nsMsgKey newParent,
            nsIDBChangeListener * aInstigator)
{
...
NS_IMETHODIMP MsgDBFolderBase::OnAnnouncerGoingAway(nsIDBChangeAnnouncer *instigator)
{
...
NS_IMETHODIMP MsgDBFolderBase::OnEvent(nsIMsgDatabase *aDB, const char *aEvent)
{
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetManyHeadersToDownload(bool *retval)
{
...
nsresult MsgDBFolderBase::MsgFitsDownloadCriteria(nsMsgKey msgKey, bool *result)
{
  if(!mDatabase)
    return NS_ERROR_FAILURE;
...
NS_IMETHODIMP MsgDBFolderBase::GetSupportsOffline(bool *aSupportsOffline)
{
...
NS_IMETHODIMP MsgDBFolderBase::ShouldStoreMsgOffline(nsMsgKey msgKey, bool *result)
{
...
NS_IMETHODIMP MsgDBFolderBase::HasMsgOffline(nsMsgKey msgKey, bool *result)
{
  NS_ENSURE_ARG(result);
  *result = false;
  GetDatabase();
...
NS_IMETHODIMP MsgDBFolderBase::GetOfflineMsgFolder(nsMsgKey msgKey, nsIMsgFolder **aMsgFolder) {
...
NS_IMETHODIMP MsgDBFolderBase::GetFlags(uint32_t *_retval)
{
  ReadDBFolderInfo(false);
  *_retval = mFlags;
  return NS_OK;
...
NS_IMETHODIMP MsgDBFolderBase::ReadFromFolderCacheElem(nsIMsgFolderCacheElement *element)
{
  nsresult rv = NS_OK;
  nsCString charset;
  ...
  element->GetInt64Property("expungedBytes", (int64_t *) &mExpungedBytes);
  element->GetInt64Property("folderSize", (int64_t *) &mFolderSize);
...
nsresult MsgDBFolderBase::GetFolderCacheKey(nsIFile **aFile, bool createDBIfMissing /* = false */)
{
  nsresult rv;
  ...
  if (dbPath)
  {
    ...
    if (!isServer)
    {
      ...
      rv = mailews::GetSummaryFileLocation(dbPath, getter_AddRefs(summaryName));
...
nsresult MsgDBFolderBase::FlushToFolderCache()
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::WriteToFolderCache(nsIMsgFolderCache *folderCache, bool deep)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP MsgDBFolderBase::WriteToFolderCacheElem(nsIMsgFolderCacheElement *element)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP
MsgDBFolderBase::AddMessageDispositionState(nsIMsgDBHdr *aMessage, nsMsgDispositionState aDispositionFlag)
{
...
nsresult MsgDBFolderBase::AddMarkAllReadUndoAction(nsIMsgWindow *msgWindow,
...
NS_IMETHODIMP
MsgDBFolderBase::MarkAllMessagesRead(nsIMsgWindow *aMsgWindow)
{
  nsresult rv = GetDatabase();
  m_newMsgs.Clear();
...
NS_IMETHODIMP MsgDBFolderBase::MarkThreadRead(nsIMsgThread *thread)
{
  nsresult rv = GetDatabase();
  if(NS_SUCCEEDED(rv))
  {
...
NS_IMETHODIMP
MsgDBFolderBase::OnStartRunningUrl(nsIURI *aUrl)
{
  return NS_OK;
}

NS_IMETHODIMP
MsgDBFolderBase::OnStopRunningUrl(nsIURI *aUrl, nsresult aExitCode)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetRetentionSettings(nsIMsgRetentionSettings **settings)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetRetentionSettings(nsIMsgRetentionSettings *settings)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetDownloadSettings(nsIMsgDownloadSettings **settings)
{
  NS_ENSURE_ARG_POINTER(settings);
  nsresult rv = NS_OK;
...
NS_IMETHODIMP MsgDBFolderBase::SetDownloadSettings(nsIMsgDownloadSettings *settings)
{
  m_downloadSettings = settings;
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::IsCommandEnabled(const nsACString& command, bool *result)
{
...
nsresult MsgDBFolderBase::WriteStartOfNewLocalMessage()
{
...
nsresult MsgDBFolderBase::StartNewOfflineMessage()
{
    bool isLocked;
    GetLocked(&isLocked);
...
nsresult MsgDBFolderBase::EndNewOfflineMessage()
{
  nsCOMPtr <nsISeekableStream> seekable;
  int64_t curStorePos;
...
nsresult MsgDBFolderBase::CompactOfflineStore(nsIMsgWindow *inWindow, nsIUrlListener *aListener)
{
  nsresult rv;
...
{
public:
  AutoCompactEvent(nsIMsgWindow *aMsgWindow, MsgDBFolderBase *aFolder)
...
private:
  ...
  nsRefPtr<MsgDBFolderBase> mFolder;
};

nsresult MsgDBFolderBase::HandleAutoCompactEvent(nsIMsgWindow *aWindow)
{
  nsresult rv;
  ...
  if (NS_SUCCEEDED(rv))
  {
    ...
    if (numServers > 0)
    {
      ...
      if (totalExpungedBytes > (purgeThreshold * 1024))
      {
        ...
        if (okToCompact)
        {
          nsCOMPtr <nsIAtom> aboutToCompactAtom = mailews::MsgGetAtom("AboutToCompact");
...
nsresult
MsgDBFolderBase::AutoCompact(nsIMsgWindow *aWindow)
{
...
NS_IMETHODIMP
MsgDBFolderBase::CompactAllOfflineStores(nsIUrlListener *aUrlListener,
...
nsresult
MsgDBFolderBase::GetPromptPurgeThreshold(bool *aPrompt)
{
...
nsresult
MsgDBFolderBase::GetPurgeThreshold(int32_t *aThreshold)
{
...
MsgDBFolderBase::MatchOrChangeFilterDestination(nsIMsgFolder *newFolder, bool caseInsensitive, bool *found)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetDBTransferInfo(nsIDBFolderInfo **aTransferInfo)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetDBTransferInfo(nsIDBFolderInfo *aTransferInfo)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetStringProperty(const char *propertyName, nsACString& propertyValue)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetStringProperty(const char *propertyName, const nsACString& propertyValue)
{
  NS_ENSURE_ARG_POINTER(propertyName);
  nsCOMPtr <nsIFile> dbPath;
...
NS_IMETHODIMP
MsgDBFolderBase::GetForcePropertyEmpty(const char *aPropertyName, bool *_retval)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetForcePropertyEmpty(const char *aPropertyName, bool aValue)
{
 nsAutoCString nameEmpty(aPropertyName);
 nameEmpty.Append(NS_LITERAL_CSTRING(".empty"));
...
NS_IMETHODIMP
MsgDBFolderBase::GetInheritedStringProperty(const char *aPropertyName, nsACString& aPropertyValue)
{
...
nsresult
MsgDBFolderBase::SpamFilterClassifyMessage(const char *aURI, nsIMsgWindow *aMsgWindow, nsIJunkMailPlugin *aJunkMailPlugin)
{
  nsresult rv;
...
nsresult
MsgDBFolderBase::SpamFilterClassifyMessages(const char **aURIArray, uint32_t aURICount, nsIMsgWindow *aMsgWindow, nsIJunkMailPlugin *aJunkMailPlugin)
{
...
NS_IMETHODIMP
MsgDBFolderBase::OnMessageClassified(const char *aMsgURI,
                                   ...
                                   uint32_t aJunkPercent)
{
  ...
  nsCOMPtr <nsIMsgDBHdr> msgHdr;
  rv = mailews::GetMsgDBHdrFromURI(aMsgURI, getter_AddRefs(msgHdr));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
MsgDBFolderBase::OnMessageTraitsClassified(const char *aMsgURI,
 ...
 */
NS_IMETHODIMP
MsgDBFolderBase::CallFilterPlugins(nsIMsgWindow *aMsgWindow, bool *aFiltersRun)
{
  ...
  *aFiltersRun = false;
  ...
  if (!classifyMsgKeys.IsEmpty())
  {
    ...
    for (uint32_t msgIndex = 0; msgIndex < numMessagesToClassify ; ++msgIndex )
    {
      ...
      if (NS_FAILED(rv))
          NS_WARNING("MsgDBFolderBase::CallFilterPlugins(): could not"
 ...
 */
nsresult MsgDBFolderBase::NotifyHdrsNotBeingClassified()
{
  ...
  if (mProcessingFlag[5].keys)
  {
    ...
    if (keys.Length())
    {
      ...
      nsresult rv = GetDatabase();
      NS_ENSURE_SUCCESS(rv, rv);
      mailews::MsgGetHeadersFromKeys(mDatabase, keys, msgHdrsNotBeingClassified);
      ...
      mProcessingFlag[5].keys = MsgKeySetU::Create();
      nsCOMPtr<nsIMsgFolderNotificationService>
...
NS_IMETHODIMP
MsgDBFolderBase::GetLastMessageLoaded(nsMsgKey *aMsgKey)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetLastMessageLoaded(nsMsgKey aMsgKey)
{
...
bool MsgDBFolderBase::PromptForMasterPasswordIfNecessary()
{
  nsresult rv;
  nsCOMPtr<nsIMsgAccountManager> accountManager =
    do_GetService(NS_MSGACCOUNTMANAGER_CONTRACTID, &rv);
...
nsresult MsgDBFolderBase::PerformBiffNotifications(void)
{
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
MsgDBFolderBase::initializeStrings()
{
  nsresult rv;
...
nsresult
MsgDBFolderBase::createCollationKeyGenerator()
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP
MsgDBFolderBase::Init(const char* aURI)
{
...
nsresult MsgDBFolderBase::CreateBaseMessageURI(const nsACString& aURI)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetURI(nsACString& name)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetSubFolders(nsISimpleEnumerator **aResult)
{
  return aResult ? NS_NewArrayEnumerator(aResult, mSubFolders) : NS_ERROR_NULL_POINTER;
}

NS_IMETHODIMP
MsgDBFolderBase::FindSubFolder(const nsACString& aEscapedSubFolderName, nsIMsgFolder **aFolder)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP
MsgDBFolderBase::GetHasSubFolders(bool *_retval)
{
  NS_ENSURE_ARG_POINTER(_retval);
...
NS_IMETHODIMP
MsgDBFolderBase::GetNumSubFolders(uint32_t *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP MsgDBFolderBase::AddFolderListener(nsIFolderListener * listener)
{
  return mListeners.AppendElement(listener) ? NS_OK : NS_ERROR_OUT_OF_MEMORY;
}

NS_IMETHODIMP MsgDBFolderBase::RemoveFolderListener(nsIFolderListener * listener)
{
  mListeners.RemoveElement(listener);
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::SetParent(nsIMsgFolder *aParent)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetParent(nsIMsgFolder **aParent)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetMessages(nsISimpleEnumerator **result)
{
...
NS_IMETHODIMP
MsgDBFolderBase::UpdateFolder(nsIMsgWindow *)
{
  return NS_OK;
}
...
NS_IMETHODIMP MsgDBFolderBase::GetFolderURL(nsACString& url)
{
  url.Assign(EmptyCString());
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetServer(nsIMsgIncomingServer ** aServer)
{
...
nsresult
MsgDBFolderBase::parseURI(bool needServer)
{
  nsresult rv;
  nsCOMPtr<nsIURL> url;
  ...
  if (mName.IsEmpty())
  {
    ...
    if (!escapedFileName.IsEmpty())
    {
      ...
      mailews::MsgUnescapeString(escapedFileName, 0, fileName);
      NS_ASSERTION(mailews::MsgIsUTF8(fileName), "fileName is not in UTF-8");
  ...
  if (server)
  {
    ...
    {
      mailews::MsgUnescapeString(escapedUrlPath, 0, urlPath);
      ...
      mailews::NS_MsgCreatePathStringFromFolderURI(urlPath.get(), newPath, scheme,
                                          isNewsFolder);
    }
...
#if defined(XP_WIN)
        mailews::MsgReplaceChar(newPath, '/', '\\');
#endif
...
NS_IMETHODIMP
MsgDBFolderBase::GetIsServer(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetNoSelect(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
  *aResult = false;
  return NS_OK;
...
NS_IMETHODIMP
MsgDBFolderBase::GetImapShared(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanSubscribe(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanFileMessages(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanDeleteMessages(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
  *aResult = true;
  return NS_OK;
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanCreateSubfolders(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanRename(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgDBFolderBase::GetCanCompact(bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
  bool isServer = false;
  nsresult rv = GetIsServer(&isServer);
...
NS_IMETHODIMP MsgDBFolderBase::GetPrettyName(nsAString& name)
{
  return GetName(name);
}

NS_IMETHODIMP MsgDBFolderBase::SetPrettyName(const nsAString& name)
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::GetName(nsAString& name)
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::SetName(const nsAString& name)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetAbbreviatedName(nsAString& aAbbreviatedName)
{
  return GetName(aAbbreviatedName);
}

NS_IMETHODIMP
MsgDBFolderBase::GetChildNamed(const nsAString& aName, nsIMsgFolder **aChild)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetChildWithURI(const nsACString& uri, bool deep, bool caseInsensitive, nsIMsgFolder ** child)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetPrettiestName(nsAString& name)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetShowDeletedMessages(bool *showDeletedMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::Delete()
{
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::DeleteSubFolders(nsIArray *folders,
...
NS_IMETHODIMP MsgDBFolderBase::CreateStorageIfMissing(nsIUrlListener* /* urlListener */)
{
  NS_ASSERTION(false, "needs to be overridden");
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::PropagateDelete(nsIMsgFolder *folder, bool deleteStorage, nsIMsgWindow *msgWindow)
{
...
NS_IMETHODIMP MsgDBFolderBase::RecursiveDelete(bool deleteStorage, nsIMsgWindow *msgWindow)
{
...
NS_IMETHODIMP MsgDBFolderBase::CreateSubfolder(const nsAString& folderName, nsIMsgWindow *msgWindow)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::AddSubfolder(const nsAString& name,
                                          nsIMsgFolder** child)
{
  NS_ENSURE_ARG_POINTER(child);
...
NS_IMETHODIMP MsgDBFolderBase::Compact(nsIUrlListener *aListener, nsIMsgWindow *aMsgWindow)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::CompactAll(nsIUrlListener *aListener, nsIMsgWindow *aMsgWindow, bool aCompactOfflineAlso)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::EmptyTrash(nsIMsgWindow *msgWindow, nsIUrlListener *aListener)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
nsresult
MsgDBFolderBase::CheckIfFolderExists(const nsAString& newFolderName, nsIMsgFolder *parentFolder, nsIMsgWindow *msgWindow)
{
...
bool
MsgDBFolderBase::ConfirmAutoFolderRename(nsIMsgWindow *msgWindow,
...
nsresult
MsgDBFolderBase::AddDirectorySeparator(nsIFile *path)
{
...
/* Finds the directory associated with this folder.  That is if the path is
  ...
  */
nsresult MsgDBFolderBase::CreateDirectoryForFolder(nsIFile **resultFile)
{
  nsresult rv = NS_OK;
...
/* Finds the backup directory associated with this folder, stored on the temp
  ...
  */
nsresult MsgDBFolderBase::CreateBackupDirectory(nsIFile **resultFile)
{
  nsCOMPtr<nsIFile> path;
...
nsresult MsgDBFolderBase::GetBackupSummaryFile(nsIFile **aBackupFile, const nsACString& newName)
{
  nsCOMPtr<nsIFile> backupDir;
...
NS_IMETHODIMP MsgDBFolderBase::Rename(const nsAString& aNewName, nsIMsgWindow *msgWindow)
{
...
NS_IMETHODIMP MsgDBFolderBase::RenameSubFolders(nsIMsgWindow *msgWindow, nsIMsgFolder *oldFolder)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::ContainsChildNamed(const nsAString& name, bool* containsChild)
{
...
NS_IMETHODIMP MsgDBFolderBase::IsAncestorOf(nsIMsgFolder *child, bool *isAncestor)
{
...
NS_IMETHODIMP MsgDBFolderBase::GenerateUniqueSubfolderName(const nsAString& prefix,
...
NS_IMETHODIMP MsgDBFolderBase::UpdateSummaryTotals(bool force)
{
...
NS_IMETHODIMP MsgDBFolderBase::SummaryChanged()
{
  UpdateSummaryTotals(false);
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetNumUnread(bool deep, int32_t *numUnread)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetTotalMessages(bool deep, int32_t *totalMessages)
{
...
int32_t MsgDBFolderBase::GetNumPendingUnread()
{
  return mNumPendingUnreadMessages;
}

int32_t MsgDBFolderBase::GetNumPendingTotalMessages()
{
  return mNumPendingTotalMessages;
}

void MsgDBFolderBase::ChangeNumPendingUnread(int32_t delta)
{
  if (delta)
  {
    int32_t oldUnreadMessages = mNumUnreadMessages + mNumPendingUnreadMessages;
...
void MsgDBFolderBase::ChangeNumPendingTotalMessages(int32_t delta)
{
  if (delta)
  {
    int32_t oldTotalMessages = mNumTotalMessages + mNumPendingTotalMessages;
...
NS_IMETHODIMP MsgDBFolderBase::SetFlag(uint32_t flag)
{
...
NS_IMETHODIMP MsgDBFolderBase::ClearFlag(uint32_t flag)
{
  // OnFlagChange can be expensive, so don't call it if we don't need to
  bool flagSet;
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::GetFlag(uint32_t flag, bool *_retval)
{
  *_retval = ((mFlags & flag) != 0);
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::ToggleFlag(uint32_t flag)
{
  mFlags ^= flag;
  OnFlagChange (flag);
...
NS_IMETHODIMP MsgDBFolderBase::OnFlagChange(uint32_t flag)
{
  nsresult rv = NS_OK;
  nsCOMPtr<nsIMsgDatabase> db;
  nsCOMPtr<nsIDBFolderInfo> folderInfo;
...
NS_IMETHODIMP MsgDBFolderBase::SetFlags(uint32_t aFlags)
{
  if (mFlags != aFlags)
  {
...
NS_IMETHODIMP MsgDBFolderBase::GetFolderWithFlags(uint32_t aFlags, nsIMsgFolder** aResult)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetFoldersWithFlags(uint32_t aFlags, nsIArray** aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP MsgDBFolderBase::ListFoldersWithFlags(uint32_t aFlags, nsIMutableArray* aFolders)
{
...
NS_IMETHODIMP MsgDBFolderBase::IsSpecialFolder(uint32_t aFlags,
...
NS_IMETHODIMP MsgDBFolderBase::GetDeletable(bool *deletable)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetDisplayRecipients(bool *displayRecipients)
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::AcquireSemaphore(nsISupports *semHolder)
{
  nsresult rv = NS_OK;
...
NS_IMETHODIMP MsgDBFolderBase::ReleaseSemaphore(nsISupports *semHolder)
{
...
NS_IMETHODIMP MsgDBFolderBase::TestSemaphore(nsISupports *semHolder, bool *result)
{
  NS_ENSURE_ARG_POINTER(result);
...
NS_IMETHODIMP MsgDBFolderBase::GetLocked(bool *isLocked)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetRelativePathName(nsACString& pathName)
{
  pathName.Truncate();
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::GetSizeOnDisk(int64_t *size)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetSizeOnDisk(int64_t aSizeOnDisk)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetUsername(nsACString& userName)
{
  nsresult rv;
  nsCOMPtr <nsIMsgIncomingServer> server;
  rv = GetServer(getter_AddRefs(server));
...
NS_IMETHODIMP MsgDBFolderBase::GetHostname(nsACString& hostName)
{
  nsresult rv;
  nsCOMPtr<nsIMsgIncomingServer> server;
  rv = GetServer(getter_AddRefs(server));
...
NS_IMETHODIMP MsgDBFolderBase::GetNewMessages(nsIMsgWindow *, nsIUrlListener * /* aListener */)
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::GetBiffState(uint32_t *aBiffState)
{
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP MsgDBFolderBase::SetBiffState(uint32_t aBiffState)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetNumNewMessages(bool deep, int32_t *aNumNewMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::SetNumNewMessages(int32_t aNumNewMessages)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetRootFolder(nsIMsgFolder * *aRootFolder)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetFilePath(nsIFile *aFile)
{
  mPath = aFile;
  return NS_OK;
}
...
NS_IMETHODIMP
MsgDBFolderBase::GetFilePath(nsIFile * *aFile)
{
...
NS_IMETHODIMP
MsgDBFolderBase::MarkMessagesRead(nsIArray *messages, bool markRead)
{
  uint32_t count;
  nsresult rv;
...
NS_IMETHODIMP
MsgDBFolderBase::MarkMessagesFlagged(nsIArray *messages, bool markFlagged)
{
  uint32_t count;
  nsresult rv;
...
NS_IMETHODIMP
MsgDBFolderBase::SetLabelForMessages(nsIArray *aMessages, nsMsgLabelValue aLabel)
{
...
NS_IMETHODIMP
MsgDBFolderBase::SetJunkScoreForMessages(nsIArray *aMessages, const nsACString& junkScore)
{
  NS_ENSURE_ARG(aMessages);
  nsresult rv = NS_OK;
  GetDatabase();
...
NS_IMETHODIMP
MsgDBFolderBase::ApplyRetentionSettings()
{
  return ApplyRetentionSettings(true);
}

nsresult MsgDBFolderBase::ApplyRetentionSettings(bool deleteViaFolder)
{
...
NS_IMETHODIMP
MsgDBFolderBase::DeleteMessages(nsIArray *messages,
                              nsIMsgWindow *msgWindow,
                              bool deleteStorage,
                              bool isMove,
...
NS_IMETHODIMP
MsgDBFolderBase::CopyMessages(nsIMsgFolder* srcFolder,
                          nsIArray *messages,
                          bool isMove,
                          nsIMsgWindow *window,
                          nsIMsgCopyServiceListener* listener,
...
NS_IMETHODIMP
MsgDBFolderBase::CopyFolder(nsIMsgFolder* srcFolder,
                        bool isMoveFolder,
                        nsIMsgWindow *window,
                        nsIMsgCopyServiceListener* listener)
{
...
NS_IMETHODIMP
MsgDBFolderBase::CopyFileMessage(nsIFile* aFile,
...
NS_IMETHODIMP MsgDBFolderBase::CopyDataToOutputStreamForAppend(nsIInputStream *aInStream,
...
NS_IMETHODIMP MsgDBFolderBase::CopyDataDone()
{
  return NS_OK;
}

NS_IMETHODIMP
MsgDBFolderBase::NotifyPropertyChanged(nsIAtom *aProperty,
...
NS_IMETHODIMP
MsgDBFolderBase::NotifyUnicharPropertyChanged(nsIAtom *aProperty,
...
NS_IMETHODIMP
MsgDBFolderBase::NotifyIntPropertyChanged(nsIAtom *aProperty, int64_t aOldValue,
                                        int64_t aNewValue)
{
...
NS_IMETHODIMP
MsgDBFolderBase::NotifyBoolPropertyChanged(nsIAtom* aProperty,
...
NS_IMETHODIMP
MsgDBFolderBase::NotifyPropertyFlagChanged(nsIMsgDBHdr *aItem, nsIAtom *aProperty,
...
NS_IMETHODIMP MsgDBFolderBase::NotifyItemAdded(nsISupports *aItem)
{
...
nsresult MsgDBFolderBase::NotifyItemRemoved(nsISupports *aItem)
{
  NS_OBSERVER_ARRAY_NOTIFY_OBSERVERS(mListeners, nsIFolderListener,
                                     OnItemRemoved,
                                     (this, aItem));
...
nsresult MsgDBFolderBase::NotifyFolderEvent(nsIAtom* aEvent)
{
  NS_OBSERVER_ARRAY_NOTIFY_OBSERVERS(mListeners, nsIFolderListener,
                                     OnItemEvent,
                                     (this, aEvent));
...
NS_IMETHODIMP
MsgDBFolderBase::GetFilterList(nsIMsgWindow *aMsgWindow, nsIMsgFilterList **aResult)
{
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
MsgDBFolderBase::SetFilterList(nsIMsgFilterList *aFilterList)
{
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP
MsgDBFolderBase::GetEditableFilterList(nsIMsgWindow *aMsgWindow, nsIMsgFilterList **aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
...
NS_IMETHODIMP
MsgDBFolderBase::SetEditableFilterList(nsIMsgFilterList *aFilterList)
{
  nsCOMPtr<nsIMsgIncomingServer> server;
  nsresult rv = GetServer(getter_AddRefs(server));
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP MsgDBFolderBase::EnableNotifications(int32_t notificationType, bool enable, bool dbBatching)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetMessageHeader(nsMsgKey msgKey, nsIMsgDBHdr **aMsgHdr)
{
  NS_ENSURE_ARG_POINTER(aMsgHdr);
  nsCOMPtr <nsIMsgDatabase> database;
...
NS_IMETHODIMP MsgDBFolderBase::GetDescendants(nsIArray** aDescendants)
{
...
NS_IMETHODIMP MsgDBFolderBase::ListDescendants(nsIMutableArray *aDescendants)
{
  NS_ENSURE_ARG_POINTER(aDescendants);
...
NS_IMETHODIMP MsgDBFolderBase::GetBaseMessageURI(nsACString& baseMessageURI)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetUriForMsg(nsIMsgDBHdr *msgHdr, nsACString& aURI)
{
...
NS_IMETHODIMP MsgDBFolderBase::GenerateMessageURI(nsMsgKey msgKey, nsACString& aURI)
{
  nsCString uri;
...
nsresult
MsgDBFolderBase::GetBaseStringBundle(nsIStringBundle **aBundle)
{
...
MsgDBFolderBase::GetStringFromBundle(const char *msgName, nsString& aResult)
{
  nsresult rv;
  nsCOMPtr <nsIStringBundle> bundle;
  rv = GetBaseStringBundle(getter_AddRefs(bundle));
...
nsresult
MsgDBFolderBase::ThrowConfirmationPrompt(nsIMsgWindow *msgWindow, const nsAString& confirmString, bool *confirmed)
{
  if (msgWindow)
  {
...
NS_IMETHODIMP
MsgDBFolderBase::GetStringWithFolderNameFromBundle(const char * msgName, nsAString& aResult)
{
  nsCOMPtr <nsIStringBundle> bundle;
  nsresult rv = GetBaseStringBundle(getter_AddRefs(bundle));
  if (NS_SUCCEEDED(rv) && bundle)
...
NS_IMETHODIMP MsgDBFolderBase::ConfirmFolderDeletionForFilter(nsIMsgWindow *msgWindow, bool *confirmed)
{
  nsString confirmString;
...
NS_IMETHODIMP MsgDBFolderBase::ThrowAlertMsg(const char * msgName, nsIMsgWindow *msgWindow)
{
...
NS_IMETHODIMP MsgDBFolderBase::AlertFilterChanged(nsIMsgWindow *msgWindow)
{
...
nsresult
MsgDBFolderBase::GetWarnFilterChanged(bool *aVal)
{
  NS_ENSURE_ARG(aVal);
  nsresult rv;
  nsCOMPtr<nsIPrefBranch> prefBranch = do_GetService(NS_PREFSERVICE_CONTRACTID, &rv);
...
nsresult
MsgDBFolderBase::SetWarnFilterChanged(bool aVal)
{
  nsresult rv=NS_OK;
  nsCOMPtr<nsIPrefBranch> prefBranch = do_GetService(NS_PREFSERVICE_CONTRACTID, &rv);
  NS_ENSURE_SUCCESS(rv, rv);
...
NS_IMETHODIMP MsgDBFolderBase::NotifyCompactCompleted()
{
  NS_ASSERTION(false, "should be overridden by child class");
  return NS_ERROR_NOT_IMPLEMENTED;
}

nsresult MsgDBFolderBase::CloseDBIfFolderNotOpen()
{
  nsresult rv;
...
NS_IMETHODIMP MsgDBFolderBase::SetSortOrder(int32_t order)
{
  NS_ASSERTION(false, "not implemented");
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP MsgDBFolderBase::GetSortOrder(int32_t *order)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetSortKey(uint32_t *aLength, uint8_t **aKey)
{
  NS_ENSURE_ARG(aKey);
  int32_t order;
...
nsresult
MsgDBFolderBase::CreateCollationKey(const nsString &aSource,  uint8_t **aKey, uint32_t *aLength)
{
...
NS_IMETHODIMP MsgDBFolderBase::CompareSortKeys(nsIMsgFolder *aFolder, int32_t *sortOrder)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetInVFEditSearchScope (bool *aInVFEditSearchScope)
{
  *aInVFEditSearchScope = mInVFEditSearchScope;
  return NS_OK;
}

NS_IMETHODIMP MsgDBFolderBase::SetInVFEditSearchScope (bool aInVFEditSearchScope, bool aSetOnSubFolders)
{
...
NS_IMETHODIMP MsgDBFolderBase::FetchMsgPreviewText(nsMsgKey *aKeysToFetch, uint32_t aNumKeys,
...
NS_IMETHODIMP MsgDBFolderBase::GetMsgTextFromStream(nsIInputStream *stream, const nsACString &aCharset,
                                                  ...
                                                  nsACString &aContentType, nsACString &aMsgText)
{
  ...
  mailews::ConvertToUnicode(charset.get(), msgText, unicodeMsgBodyStr);
  ...
  if (aMsgText.Length() > aMaxOutputLen) {
    if (NS_IsAscii(aMsgText.BeginReading()))
      ...
      mailews::MsgI18NShrinkUTF8Str(nsCString(aMsgText),
 ...
 */
void MsgDBFolderBase::decodeMsgSnippet(const nsACString& aEncodingType, bool aIsComplete, nsCString& aMsgSnippet)
{
  ...
  else if (MsgLowerCaseEqualsLiteral(aEncodingType, ENCODING_QUOTED_PRINTABLE))
  {
    ...
    mailews::MsgStripQuotedPrintable((unsigned char *) aMsgSnippet.get());
 ...
 */
void MsgDBFolderBase::compressQuotesInMsgSnippet(const nsString& aMsgSnippet, nsAString& aCompressedQuotes)
{
...
NS_IMETHODIMP MsgDBFolderBase::ConvertMsgSnippetToPlainText(
...
nsresult MsgDBFolderBase::GetMsgPreviewTextFromStream(nsIMsgDBHdr *msgHdr, nsIInputStream *stream)
{
  nsCString msgBody;
  ...
  mailews::MsgCompressWhitespace(msgBody);
  msgHdr->SetStringProperty("preview", msgBody.get());
  return rv;
}

void MsgDBFolderBase::UpdateTimestamps(bool allowUndo)
{
  if (!(mFlags & (nsMsgFolderFlags::Trash|nsMsgFolderFlags::Junk)))
  {
    ...
    if (allowUndo) // This is a proxy for a user-initiated act.
    {
      ...
      {
        SetMRMTime();
        nsCOMPtr<nsIAtom> MRMTimeChangedAtom = mailews::MsgGetAtom("MRMTimeChanged");
...
void MsgDBFolderBase::SetMRUTime()
{
  uint32_t seconds;
  mailews::PRTime2Seconds(PR_Now(), &seconds);
  nsAutoCString nowStr;
  nowStr.AppendInt(seconds);
  SetStringProperty(MRU_TIME_PROPERTY, nowStr);
}

void MsgDBFolderBase::SetMRMTime()
{
  uint32_t seconds;
  mailews::PRTime2Seconds(PR_Now(), &seconds);
  nsAutoCString nowStr;
...
NS_IMETHODIMP MsgDBFolderBase::AddKeywordsToMessages(nsIArray *aMessages, const nsACString& aKeywords)
{
  NS_ENSURE_ARG(aMessages);
  nsresult rv = NS_OK;
  GetDatabase();
  if (mDatabase)
  {
    ...
    for(uint32_t i = 0; i < count; i++)
    {
      ...
      for (uint32_t j = 0; j < keywordArray.Length(); j++)
      {
        ...
        if (!mailews::MsgFindKeyword(keywordArray[j], keywords, &start, &length))
        {
          if (!keywords.IsEmpty())
            keywords.Append(' ');
...
NS_IMETHODIMP MsgDBFolderBase::RemoveKeywordsFromMessages(nsIArray *aMessages, const nsACString& aKeywords)
{
  NS_ENSURE_ARG(aMessages);
  nsresult rv = NS_OK;
  GetDatabase();
  if (mDatabase)
  {
    ...
    for(uint32_t i = 0; i < count; i++)
    {
      ...
      for (uint32_t j = 0; j < keywordArray.Length(); j++)
      {
        ...
        if (mailews::MsgFindKeyword(keywordArray[j], keywords, &startOffset, &length))
        {
...
NS_IMETHODIMP MsgDBFolderBase::GetCustomIdentity(nsIMsgIdentity **aIdentity)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetProcessingFlags(nsMsgKey aKey, uint32_t *aFlags)
{
...
NS_IMETHODIMP MsgDBFolderBase::OrProcessingFlags(nsMsgKey aKey, uint32_t mask)
{
  for (uint32_t i = 0; i < nsMsgProcessingFlags::NumberOfFlags; i++)
...
NS_IMETHODIMP MsgDBFolderBase::AndProcessingFlags(nsMsgKey aKey, uint32_t mask)
{
  for (uint32_t i = 0; i < nsMsgProcessingFlags::NumberOfFlags; i++)
    if (!(mProcessingFlag[i].bit & mask) && mProcessingFlag[i].keys)
...
void MsgDBFolderBase::ClearProcessingFlags()
{
  for (uint32_t i = 0; i < nsMsgProcessingFlags::NumberOfFlags; i++)
  {
...
/* static */ MsgKeySetU* MsgKeySetU::Create()
{
  MsgKeySetU* set = new MsgKeySetU;
  if (set)
  {
    set->loKeySet = MsgKeySet::Create();
    set->hiKeySet = MsgKeySet::Create();
  }
...
int MsgKeySetU::Add(uint32_t aKey)
{
...
int MsgKeySetU::Remove(uint32_t aKey)
{
  int32_t intKey = static_cast<int32_t>(aKey);
  if (intKey >= 0)
...
bool MsgKeySetU::IsMember(uint32_t aKey)
{
  int32_t intKey = static_cast<int32_t>(aKey);
  if (intKey >= 0)
...
nsresult MsgKeySetU::ToMsgKeyArray(nsTArray<nsMsgKey> &aArray)
{
...
NS_IMETHODIMP MsgDBFolderBase::GetSummaryFile(nsIFile **aSummaryFile)
{
...
NS_IMETHODIMP
MsgDBFolderBase::GetDatabaseOpen(bool *aOpen)
{
