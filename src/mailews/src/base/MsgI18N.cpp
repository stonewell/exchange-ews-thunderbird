...
namespace mailews {
nsresult MsgI18NConvertFromUnicode(const char* aCharset,
                                     ...
                                     bool aIsCharsetCanonical)
{
  ...
  // (e.g. Latin1 + Japanese). Use MsgI18NSaveAsCharset instead to avoid that problem.
...
nsresult MsgI18NConvertToUnicode(const char* aCharset,
...
const char * MsgI18NFileSystemCharset()
{
...
void MsgI18NTextFileCharset(nsACString& aCharset)
{
  nsresult rv;
...
char * MsgI18NEncodeMimePartIIStr(const char *header, bool structured, const char *charset, int32_t fieldnamelen, bool usemime) 
{
...
bool MsgI18Nstateful_charset(const char *charset)
{
  //TODO: use charset manager's service
  return (PL_strcasecmp(charset, "ISO-2022-JP") == 0);
}

bool MsgI18Nmultibyte_charset(const char *charset)
{
  nsresult res;
...
bool MsgI18Ncheck_data_in_charset_range(const char *charset, const char16_t* inString, char **fallbackCharset)
{
  ...
  if (!result && fallbackCharset) {
    ...
    res = MsgI18NSaveAsCharset("text/plain", charset, inString, 
...
const char * 
MsgI18NParseMetaCharset(nsIFile* file) 
{ 
...
nsresult MsgI18NSaveAsCharset(const char* contentType, const char *charset, 
                                ...
                                char **fallbackCharset, bool *isAsciiOnly)
{
  ...
  *outString = nullptr;
  ...
  // Exclude stateful charset which is 7 bit but not ASCII only.
           ...
           !MsgI18Nstateful_charset(charsetName.get()))
    *isAsciiOnly = NS_IsAscii(*outString);

  return res;
}

nsresult MsgI18NShrinkUTF8Str(const nsCString &inString,
                                uint32_t aMaxLength,
                                nsACString &outString)
{
  if (inString.IsEmpty()) {
...
void MsgI18NConvertRawBytesToUTF16(const nsCString& inString, 
                                     const char* charset,
                                     nsAString& outString)
{
  if (MsgIsUTF8(inString))
...
void MsgI18NConvertRawBytesToUTF8(const nsCString& inString, 
                                    const char* charset,
                                    nsACString& outString)
{
  if (MsgIsUTF8(inString))
