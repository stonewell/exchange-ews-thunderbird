...
MsgIncomingServerBase::MsgIncomingServerBase():
    m_rootFolder(0),
    m_downloadedHdrs(50),
...
NS_IMPL_ISUPPORTS(MsgIncomingServerBase, nsIMsgIncomingServer,
  nsISupportsWeakReference)

NS_IMETHODIMP
MsgIncomingServerBase::SetServerBusy(bool aServerBusy)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetServerBusy(bool * aServerBusy)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetKey(nsACString& serverKey)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetKey(const nsACString& serverKey)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetRootFolder(nsIMsgFolder * aRootFolder)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetRootFolder(nsIMsgFolder * *aRootFolder)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetRootMsgFolder(nsIMsgFolder **aRootMsgFolder)
{
  return GetRootFolder(aRootMsgFolder);
}

NS_IMETHODIMP
MsgIncomingServerBase::PerformExpand(nsIMsgWindow *aMsgWindow)
{
  return NS_OK;
}

NS_IMETHODIMP
MsgIncomingServerBase::VerifyLogon(nsIUrlListener *aUrlListener, nsIMsgWindow *aMsgWindow,
                                 nsIURI **aURL)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::PerformBiff(nsIMsgWindow* aMsgWindow)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetNewMessages(nsIMsgFolder *aFolder, nsIMsgWindow *aMsgWindow,
...
NS_IMETHODIMP MsgIncomingServerBase::GetPerformingBiff(bool *aPerformingBiff)
{
...
NS_IMETHODIMP MsgIncomingServerBase::SetPerformingBiff(bool aPerformingBiff)
{
  mPerformingBiff = aPerformingBiff;
  return NS_OK;
}

NS_IMPL_GETSET(MsgIncomingServerBase, BiffState, uint32_t, m_biffState)

NS_IMETHODIMP MsgIncomingServerBase::WriteToFolderCache(nsIMsgFolderCache *folderCache)
{
  nsresult rv = NS_OK;
  if (m_rootFolder)
  {
...
NS_IMETHODIMP
MsgIncomingServerBase::GetDownloadMessagesAtStartup(bool *getMessagesAtStartup)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanHaveFilters(bool *canHaveFilters)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetCanHaveFilters(bool aCanHaveFilters)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanBeDefaultServer(bool *canBeDefaultServer)
{
  // derived class should override if they need to do this.
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanSearchMessages(bool *canSearchMessages)
{
  // derived class should override if they need to do this.
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanCompactFoldersOnServer(bool *canCompactFoldersOnServer)
{
  // derived class should override if they need to do this.
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanUndoDeleteOnServer(bool *canUndoDeleteOnServer)
{
  // derived class should override if they need to do this.
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCanEmptyTrashOnExit(bool *canEmptyTrashOnExit)
{
  // derived class should override if they need to do this.
...
NS_IMETHODIMP
MsgIncomingServerBase::GetServerURI(nsACString& aResult)
{
  nsresult rv;
  // rv = GetLocalStoreType(aResult);
  // NS_ENSURE_SUCCESS(rv, rv);
 	aResult.AssignLiteral("ews");
  ...
  if (NS_SUCCEEDED(rv) && !username.IsEmpty()) {
      ...
      mailews::MsgEscapeString(username, nsINetUtil::ESCAPE_XALPHAS, escapedUsername);
  ...
  if (NS_SUCCEEDED(rv) && !hostname.IsEmpty()) {
      ...
      mailews::MsgEscapeString(hostname, nsINetUtil::ESCAPE_URL_PATH, escapedHostname);
...
nsresult
MsgIncomingServerBase::CreateLocalFolder(const nsAString& folderName)
{
...
nsresult
MsgIncomingServerBase::CreateRootFolder()
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetBoolValue(const char *prefname,
                                 bool *val)
{
  if (!mPrefBranch)
...
NS_IMETHODIMP
MsgIncomingServerBase::SetBoolValue(const char *prefname,
                                 bool val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetIntValue(const char *prefname,
                                int32_t *val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetFileValue(const char* aRelPrefName,
...
NS_IMETHODIMP
MsgIncomingServerBase::SetFileValue(const char* aRelPrefName,
                                  const char* aAbsPrefName,
                                  nsIFile* aLocalFile)
{
  if (!mPrefBranch)
...
NS_IMETHODIMP
MsgIncomingServerBase::SetIntValue(const char *prefname,
                                 int32_t val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
  ...
  if (NS_SUCCEEDED(rv) && defaultVal == val)
    ...
    int32_t oldVal;
    nsresult rv1 = mPrefBranch->GetIntPref(prefname, &oldVal);

    rv = mPrefBranch->SetIntPref(prefname, val);

    if (!strcmp(prefname, "auth")) {
      if (NS_FAILED(rv1) || oldVal != val) {
        ForgetPassword();
...
NS_IMETHODIMP
MsgIncomingServerBase::GetCharValue(const char *prefname,
                                  nsACString& val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetUnicharValue(const char *prefname,
                                     nsAString& val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::SetCharValue(const char *prefname,
                                  const nsACString& val)
{
  ...
  if (NS_SUCCEEDED(rv) && defaultVal.Equals(val))
    ...
    nsCString oldVal;
    nsresult rv1 = mPrefBranch->GetCharPref(prefname, getter_Copies(oldVal));
    
    rv = mPrefBranch->SetCharPref(prefname, nsCString(val).get());
    
    if (!strcmp(prefname, "url")) {
      if (NS_FAILED(rv1) || !oldVal.Equals(nsCString(val))) {
        ForgetPassword();
        CloseCachedConnections();
      }
    }
...
NS_IMETHODIMP
MsgIncomingServerBase::SetUnicharValue(const char *prefname,
                                     const nsAString& val)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetPrettyName(nsAString& retval)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetPrettyName(const nsAString& value)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetConstructedPrettyName(nsAString& retval)
{
  nsCString username;
...
NS_IMETHODIMP
MsgIncomingServerBase::ToString(nsAString& aResult)
{
...
NS_IMETHODIMP MsgIncomingServerBase::SetPassword(const nsACString& aPassword)
{
  m_password = aPassword;
  return NS_OK;
}

NS_IMETHODIMP MsgIncomingServerBase::GetPassword(nsACString& aPassword)
{
  aPassword = m_password;
  return NS_OK;
}

NS_IMETHODIMP MsgIncomingServerBase::GetServerRequiresPasswordForBiff(bool *aServerRequiresPasswordForBiff)
{
...
nsresult MsgIncomingServerBase::GetPasswordWithoutUI()
{
  nsresult rv;
  ...
  // NS_ENSURE_SUCCESS(rv, rv);
 	currServerUri.AssignLiteral("ews");
...
NS_IMETHODIMP
MsgIncomingServerBase::GetPasswordWithUI(const nsAString& aPromptMessage, const
                                       ...
                                       nsACString& aPassword)
{
  ...
  if (m_password.IsEmpty())
  {
    ...
    if (dialog)
    {
      ...
      {
        nsCString escapedUsername;
        mailews::MsgEscapeString(temp, nsINetUtil::ESCAPE_XALPHAS, escapedUsername);
...
NS_IMETHODIMP
MsgIncomingServerBase::ForgetPassword()
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::ForgetSessionPassword()
{
  m_password.Truncate();
  return NS_OK;
}
...
NS_IMETHODIMP
MsgIncomingServerBase::SetDefaultLocalPath(nsIFile *aDefaultLocalPath)
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetLocalPath(nsIFile **aLocalPath)
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetMsgStore(nsIMsgPluggableStore **aMsgStore)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetLocalPath(nsIFile *aLocalPath)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetLocalStoreType(nsACString& aResult)
{
  NS_NOTYETIMPLEMENTED("MsgIncomingServerBase superclass not implementing GetLocalStoreType!");
  return NS_ERROR_UNEXPECTED;
}

NS_IMETHODIMP
MsgIncomingServerBase::GetAccountManagerChrome(nsAString& aResult)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::Equals(nsIMsgIncomingServer *server, bool *_retval)
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::ClearAllValues()
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::RemoveFiles()
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetFilterList(nsIMsgFilterList *aFilterList)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetFilterList(nsIMsgWindow *aMsgWindow, nsIMsgFilterList **aResult)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetEditableFilterList(nsIMsgFilterList *aEditableFilterList)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetEditableFilterList(nsIMsgWindow *aMsgWindow, nsIMsgFilterList **aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
nsresult
MsgIncomingServerBase::InternalSetHostName(const nsACString& aHostname, const char * prefName)
{
  nsCString hostname;
  hostname = aHostname;
  if (mailews::MsgCountChar(hostname, ':') == 1)
  {
...
NS_IMETHODIMP
MsgIncomingServerBase::OnUserOrHostNameChanged(const nsACString& oldName,
...
NS_IMETHODIMP
MsgIncomingServerBase::SetHostName(const nsACString& aHostname)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetRealHostName(const nsACString& aHostname)
{
  nsCString oldName;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetHostName(nsACString& aResult)
{
  nsresult rv;
  rv = GetCharValue("hostname", aResult);
  if (mailews::MsgCountChar(aResult, ':') == 1)
  {
...
NS_IMETHODIMP
MsgIncomingServerBase::GetRealHostName(nsACString& aResult)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetRealUsername(nsACString& aResult)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetRealUsername(const nsACString& aUsername)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetDoBiff(bool *aDoBiff)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetDoBiff(bool aDoBiff)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetPort(int32_t *aPort)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetPort(int32_t aPort)
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetProtocolInfo(nsIMsgProtocolInfo **aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP MsgIncomingServerBase::GetRetentionSettings(nsIMsgRetentionSettings **settings)
{
...
NS_IMETHODIMP MsgIncomingServerBase::SetRetentionSettings(nsIMsgRetentionSettings *settings)
{
  nsMsgRetainByPreference retainByPreference;
...
NS_IMETHODIMP
MsgIncomingServerBase::GetDisplayStartupPage(bool *displayStartupPage)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetDisplayStartupPage(bool displayStartupPage)
{
...
NS_IMETHODIMP MsgIncomingServerBase::GetDownloadSettings(nsIMsgDownloadSettings **settings)
{
  NS_ENSURE_ARG_POINTER(settings);
...
NS_IMETHODIMP MsgIncomingServerBase::SetDownloadSettings(nsIMsgDownloadSettings *settings)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetSupportsDiskSpace(bool *aSupportsDiskSpace)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetOfflineSupportLevel(int32_t *aSupportLevel)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::SetOfflineSupportLevel(int32_t aSupportLevel)
{
...
NS_IMETHODIMP MsgIncomingServerBase::DisplayOfflineMsg(nsIMsgWindow *aMsgWindow)
{
...
// migrated account.
NS_IMETHODIMP
MsgIncomingServerBase::GeneratePrettyNameForMigration(nsAString& aPrettyName)
{
/**
...
NS_IMETHODIMP
MsgIncomingServerBase::GetFilterScope(nsMsgSearchScopeValue *filterScope)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetSearchScope(nsMsgSearchScopeValue *searchScope)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetIsSecure(bool *aIsSecure)
{
...
NS_IMPL_SERVERPREF_STR(MsgIncomingServerBase, Username, "userName")
NS_IMPL_SERVERPREF_INT(MsgIncomingServerBase, AuthMethod, "authMethod")
NS_IMPL_SERVERPREF_INT(MsgIncomingServerBase, BiffMinutes, "check_time")
NS_IMPL_SERVERPREF_STR(MsgIncomingServerBase, Type, "type")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, DownloadOnBiff, "download_on_biff")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, Valid, "valid")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, EmptyTrashOnExit,
                        "empty_trash_on_exit")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, CanDelete, "canDelete")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, LoginAtStartUp, "login_at_startup")
NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase,
...
NS_IMPL_SERVERPREF_INT(MsgIncomingServerBase, MaxMessageSize, "max_size")

NS_IMPL_SERVERPREF_INT(MsgIncomingServerBase, IncomingDuplicateAction, "dup_action")

NS_IMPL_SERVERPREF_BOOL(MsgIncomingServerBase, Hidden, "hidden")

NS_IMETHODIMP MsgIncomingServerBase::GetSocketType(int32_t *aSocketType)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
  ...
  if (NS_FAILED(rv))
  {
    ...
    if (NS_SUCCEEDED(rv) && isSecure)
    {
      *aSocketType = nsMsgSocketType::SSL;
      ...
      MsgIncomingServerBase::SetSocketType(*aSocketType);
    }
    else
    {
      if (!mDefPrefBranch)
...
NS_IMETHODIMP MsgIncomingServerBase::SetSocketType(int32_t aSocketType)
{
  if (!mPrefBranch)
    return NS_ERROR_NOT_INITIALIZED;
  ...
  if ((isSecureOld != isSecureNew) && m_rootFolder) {
    nsCOMPtr <nsIAtom> isSecureAtom = mailews::MsgGetAtom("isSecure");
...
NS_IMETHODIMP
MsgIncomingServerBase::GetPasswordPromptRequired(bool *aPasswordIsRequired)
{
...
NS_IMETHODIMP MsgIncomingServerBase::ConfigureTemporaryFilters(nsIMsgFilterList *aFilterList)
{
...
nsresult
MsgIncomingServerBase::ConfigureTemporaryServerSpamFilters(nsIMsgFilterList *filterList)
{
  ...
  if (newFilter && serverFilterTrustFlags & nsISpamSettings::TRUST_POSITIVES)
  {
    ...
    if (count > 1) // don't need to group a single term
    {
      // beginGrouping the first term, and endGrouping the last term
	    nsCOMPtr<nsIMsgSearchTerm> firstTerm(mailews::do_QueryElementAt(searchTerms,
                                                             0, &rv));
      NS_ENSURE_SUCCESS(rv,rv);
      firstTerm->SetBeginsGrouping(true);

      nsCOMPtr<nsIMsgSearchTerm> lastTerm(mailews::do_QueryElementAt(searchTerms,
                                                            count - 1, &rv));
      NS_ENSURE_SUCCESS(rv,rv);
...
nsresult
MsgIncomingServerBase::ConfigureTemporaryReturnReceiptsFilter(nsIMsgFilterList *filterList)
{
  nsresult rv;
...
NS_IMETHODIMP
MsgIncomingServerBase::ClearTemporaryReturnReceiptsFilter()
{
  if (mFilterList)
  {
...
NS_IMETHODIMP
MsgIncomingServerBase::GetMsgFolderFromURI(nsIMsgFolder *aFolderResource, const nsACString& aURI, nsIMsgFolder **aFolder)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetSpamSettings(nsISpamSettings **aSpamSettings)
{
...
NS_IMETHODIMP
MsgIncomingServerBase::GetSpamFilterPlugin(nsIMsgFilterPlugin **aFilterPlugin)
{
...
nsresult MsgIncomingServerBase::GetDeferredServers(nsIMsgIncomingServer *destServer, nsCOMArray<nsIPop3IncomingServer>& aServers)
{
  nsresult rv;
...
NS_IMETHODIMP MsgIncomingServerBase::GetIsDeferredTo(bool *aIsDeferredTo)
{
...
/* static */PLDHashOperator MsgIncomingServerBase::evictOldEntries(nsCStringHashKey::KeyType aKey, int32_t &aData, void *aClosure)
{
  MsgIncomingServerBase *server = (MsgIncomingServerBase *) aClosure;
...
NS_IMETHODIMP MsgIncomingServerBase::IsNewHdrDuplicate(nsIMsgDBHdr *aNewHdr, bool *aResult)
{
  NS_ENSURE_ARG_POINTER(aResult);
...
NS_IMETHODIMP
MsgIncomingServerBase::GetForcePropertyEmpty(const char *aPropertyName, bool *_retval)
{
  NS_ENSURE_ARG_POINTER(_retval);
...
NS_IMETHODIMP
MsgIncomingServerBase::SetForcePropertyEmpty(const char *aPropertyName, bool aValue)
{
 nsAutoCString nameEmpty(aPropertyName);
 nameEmpty.Append(NS_LITERAL_CSTRING(".empty"));
...
NS_IMETHODIMP
MsgIncomingServerBase::GetSortOrder(int32_t* aSortOrder)
{
