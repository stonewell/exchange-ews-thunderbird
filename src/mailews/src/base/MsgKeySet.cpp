...
MsgKeySet::MsgKeySet(/* MSG_NewsHost* host*/)
{
  MOZ_COUNT_CTOR(MsgKeySet);
  m_cached_value = -1;
...
MsgKeySet::MsgKeySet(const char* numbers /* , MSG_NewsHost* host */)
{
...
MsgKeySet*
MsgKeySet::Create(/*MSG_NewsHost* host*/)
{
  MsgKeySet* set = new MsgKeySet(/* host */);
...
MsgKeySet*
MsgKeySet::Create(const char* value /* , MSG_NewsHost* host */)
{
#ifdef DEBUG_MSGKEYSET
    printf("create from %s\n",value);
#endif

  MsgKeySet* set = new MsgKeySet(value /* , host */);
  if (set && set->m_data == NULL) {
    delete set;
    set = NULL;
  }
...
nsresult 
MsgKeySet::Output(char **outputStr)
{
...
void MsgKeySet::SetLastMember(int32_t newHighWaterMark)
{
...
/* Re-compresses a `MsgKeySet' object.

   The assumption is made that the `MsgKeySet' is syntactically correct
...
bool
MsgKeySet::IsMember(int32_t number)
{
  bool value = false;
  int32_t size;
  int32_t *head;
...
int
MsgKeySet::Remove(int32_t number)
{
  int32_t size;
  int32_t *head;
  int32_t *tail;
...
int
MsgKeySet::AddRange(int32_t start, int32_t end)
{
  int32_t tmplength;
  int32_t* tmp;
  int32_t* in;
...
int32_t
MsgKeySet::CountMissingInRange(int32_t range_start, int32_t range_end)
{
  int32_t count;
  int32_t *head;
  int32_t *tail;
...
int 
MsgKeySet::FirstMissingRange(int32_t min, int32_t max,
...
int 
MsgKeySet::LastMissingRange(int32_t min, int32_t max,
                  int32_t* first, int32_t* last)
{
  int32_t size;
  int32_t *head;
 ...
 */
nsresult
MsgKeySet::ToMsgKeyArray(nsTArray<nsMsgKey> &aArray)
{
    int32_t size;
    int32_t *head;
    int32_t *tail;
...
void
MsgKeySet::test_decoder (const char *string)
{
  MsgKeySet set(string /* , NULL */);
  char* tmp;
  set.Output(&tmp);
  printf ("\t\"%s\"\t--> \"%s\"\n", string, tmp);
  nsMemory::Free(tmp);
...
#define START(STRING) \
  string = STRING;    \
  if (!(set = MsgKeySet::Create(string))) abort ()
...
void
MsgKeySet::test_member(bool with_cache)
{
  MsgKeySet *set;
  char *s;
