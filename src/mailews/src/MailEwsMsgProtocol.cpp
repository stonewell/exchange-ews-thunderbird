...
MailEwsMsgProtocol::MailEwsMsgProtocol(nsIURI * aURI)
    : MsgProtocolBase(aURI)
{
  m_lineStreamBuffer =nullptr;

  // initialize the pr log if it hasn't been initialiezed already
  if (!MAILEWS)
    MAILEWS = PR_NewLogModule("MAILEWS");
}

MailEwsMsgProtocol::~MailEwsMsgProtocol()
{
  // free our local state 
  delete m_lineStreamBuffer;
}

nsresult MailEwsMsgProtocol::OpenMultipleMsgTransport(uint64_t offset, int32_t size)
{
  nsresult rv;
...
nsresult MailEwsMsgProtocol::OpenFileSocketForReuse(nsIURI * aURL, uint64_t aStartPosition, int32_t aReadCount)
{
...
nsresult MailEwsMsgProtocol::Initialize(nsIURI * aURL)
{
  NS_PRECONDITION(aURL, "invalid URL passed into MAILEWS Protocol");
  nsresult rv = NS_OK;
  if (aURL)
  {
  ...
  m_lineStreamBuffer = new MsgLineStreamBuffer(OUTPUT_BUFFER_SIZE, true);
  
  m_nextState = MAILBOX_READ_MESSAGE;
  m_initialState = MAILBOX_READ_MESSAGE;
  mCurrentProgress = 0;
...
NS_IMETHODIMP MailEwsMsgProtocol::OnStartRequest(nsIRequest *request, nsISupports *ctxt)
{
  ...
  return MsgProtocolBase::OnStartRequest(request, ctxt);
}

bool MailEwsMsgProtocol::RunningMultipleMsgUrl()
{
...
NS_IMETHODIMP MailEwsMsgProtocol::OnStopRequest(nsIRequest *request, nsISupports *ctxt, nsresult aStatus)
{
  nsresult rv;
  if (m_nextState == MAILBOX_READ_FOLDER && m_mailboxParser)
  {
  ...
  PR_LOG(MAILEWS, PR_LOG_ALWAYS, ("Mailews Done\n"));
  ...
  MsgProtocolBase::OnStopRequest(request, ctxt, aStatus);
  return CloseSocket(); 
}
...
nsresult MailEwsMsgProtocol::DoneReadingMessage()
{
  nsresult rv = NS_OK;
...
nsresult MailEwsMsgProtocol::SetupMessageExtraction()
{
...
nsresult MailEwsMsgProtocol::LoadUrl(nsIURI * aURL, nsISupports * aConsumer)
{
  nsresult rv = NS_OK;
  ...
  if (aURL)
  {
    ...
    if (m_runningUrl)
    {
        ...
        case nsIMailboxUrl::ActionSaveMessageToDisk:
          ...
          // to display the message after we write it to disk...
          {
            ...
            if (NS_SUCCEEDED(rv))
            {
              ...
              rv = mailews::MsgNewBufferedFileOutputStream(getter_AddRefs(m_msgFileOutputStream), m_tempMessageFile, -1, 00600);
              NS_ENSURE_SUCCESS(rv, rv);
      ...
      rv = MsgProtocolBase::LoadUrl(aURL, m_channelListener);
...
int32_t MailEwsMsgProtocol::ReadFolderResponse(nsIInputStream * inputStream, uint64_t sourceOffset, uint32_t length)
{
...
int32_t MailEwsMsgProtocol::ReadMessageResponse(nsIInputStream * inputStream, uint64_t sourceOffset, uint32_t length)
{
  char *line = nullptr;
  uint32_t status = 0;
  nsresult rv = NS_OK;
 ...
 */
nsresult MailEwsMsgProtocol::ProcessProtocolState(nsIURI * url, nsIInputStream * inputStream, uint64_t offset, uint32_t length)
{
  nsresult rv = NS_OK;
  int32_t status = 0;
...
nsresult MailEwsMsgProtocol::CloseSocket()
{
  // how do you force a release when closing the connection??
  MsgProtocolBase::CloseSocket(); 
