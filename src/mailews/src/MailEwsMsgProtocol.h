...
#ifndef MailEwsMsgProtocol_h___
#define MailEwsMsgProtocol_h___
...
class MsgLineStreamBuffer;

class MailEwsMsgProtocol : public MsgProtocolBase
{
public:
  // Creating a protocol instance requires the URL which needs to be run AND it requires
  // a transport layer.
  MailEwsMsgProtocol(nsIURI * aURL);
  virtual ~MailEwsMsgProtocol();
...
private:
  ...
  MsgLineStreamBuffer   * m_lineStreamBuffer; // used to efficiently extract lines from the incoming data stream
